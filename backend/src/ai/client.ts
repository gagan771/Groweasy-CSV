...
function buildSystemPrompt(): string {
  return [
    ...
    "Map semantic variants of headers, not just exact names.",
    "Examples: Business/Company/Organisation -> company; Location/City -> city; Region/Province -> state; Nation/Country -> country; Assigned To/Lead Owner/Sales Rep -> lead_owner.",
    "NEVER use Assigned To / Lead Owner / Sales Rep values as the lead's email or mobile.",
    ...
    "Use created_at from the row when available. Never invent a fixed placeholder timestamp like 2024-01-01T00:00:00.000Z.",
    "If no reliable created_at exists in the row, return an empty string for created_at.",
